...
class TestInvokeMultimodalPayload:
    ...
    async def test_content_input_forwarded_as_list(self) -> None:
        ...
        with (
            ...
            _ = [item async for item in invoke({"content": [{"text": "hello"}, {"text": "world"}]})]

        assert isinstance(captured["agent_input"], list)
        assert captured["agent_input"] == [{"text": "hello"}, {"text": "world"}]

    @pytest.mark.asyncio
    async def test_messages_input_forwarded_as_messages(self) -> None:
        app = create_app(make_app_config(), make_infra())
        invoke = app.handlers["main"]
        ...
        with (
            ...
            _ = [item async for item in invoke({"messages": msgs})]

        assert captured["agent_input"] == msgs
    ...
    @pytest.mark.asyncio
    async def test_multiagent_entry_rejects_messages(self) -> None:
        from strands.multiagent.base import MultiAgentBase

        multi_entry = MagicMock(spec=MultiAgentBase)
        resolved = make_resolved_config(entry=multi_entry)
        events = MagicMock(spec=EventQueue)
        ...
        # First event is the structured MultimodalPayloadError event.
        assert results
        assert results[-1]["type"] == "error"
        assert "multi-agent" in results[-1]["data"]["message"]
