...
The `/invocations` endpoint accepts a single top-level key, `prompt`, whose value mirrors the polymorphic `AgentInput` accepted by `strands.Agent.__call__`:

| Value shape | Use case |
|-------------|----------|
| `str` | Plain text user turn |
| One content block dict | A single image, document, text, or reply block (wrapped into a one-element list internally) |
| `list[ContentBlock]` | A single user turn that mixes text with images and documents |

Missing the `prompt` key or sending an unsupported value yields a structured `error` event, not a 500. Inside any media block, the `source` dict must use:

- `"base64": "<b64-string>"` — the server decodes it into native `bytes` before invoking the agent.

Example payload with all three block types:

```jsonc
{
  "prompt": [
    {"image": {"format": "png", "source": {"base64": "iVBORw0KGgo..."}}},
    {"document": {"format": "pdf", "name": "report", "source": {"base64": "JVBERi0x..."}}},
    {"text": "Summarise the image and the document."}
  ]
}
```

A plain-text turn uses the shorthand string form:
...
For client code, `strands_compose_agentcore.media` ships pure builders for all block types:
...
| Builder | Block type | Accepts |
|---------|------------|---------|
| `text(str)` | `{"text": ...}` | Any string |
...
| `max_media_blocks` | 20 | Max number of image and document blocks per request |

A violation yields a structured `error` event, not a 500.

### Limitation: delegate sub-agents

Delegate tools generated by `strands-compose` accept text input only, so sub-agents in a delegate orchestration cannot receive multimodal blocks today. Use a single-agent entry (or a `Swarm`/`Graph` entry) when you need to pass images or documents.
