...
The `/invocations` endpoint accepts **exactly one** of three mutually-exclusive top-level keys:

| Key | Value type | Use case |
|-----|------------|----------|
| `prompt` | `str` | Plain text user turn — back-compat default |
| `content` | `list[ContentBlock]` | A single user turn that mixes text with images, documents, or video |
| `messages` | `list[Message]` | A full multi-turn conversation (`role` + `content`) |

Sending zero or more than one of these keys yields a structured `error` event, not a 500. Inside any media block, the `source` dict accepts either:

- `"base64": "<b64-string>"` — the JSON-safe synonym. The server decodes it into native `bytes` before invoking the agent.
- `"location": {"type": "s3", "uri": "s3://...", "bucketOwner": "..."}` — passed through unchanged to Bedrock.
...
For client code, `strands_compose_agentcore.media` ships pure builders that produce the right shape:

```python
from strands_compose_agentcore import LocalClient, image_block, s3_document_block

client = LocalClient()
for event in client.invoke(content=[image_block("cat.png"),
                                    {"text": "Describe"}]):
...
`create_app()` exposes three limits enforced by the payload parser:

| Parameter | Default | Purpose |
|-----------|---------|---------|
| `max_payload_bytes` | 25 MiB | Total JSON-serialized request body. `None` disables. |
| `max_media_bytes` | 20 MiB | Max decoded bytes for any single `source.base64` block |
| `max_media_blocks` | 20 | Max number of media blocks (including S3 references) per request |

A violation yields a structured `error` event, not a 500.

### Limitation: delegate sub-agents

Delegate tools generated by `strands-compose` accept text input only, so sub-agents in a delegate orchestration cannot receive multimodal blocks today. Use a single-agent entry (or a `Swarm`/`Graph` entry with `content`) when you need to pass images or documents. `MultiAgentBase` entries reject full `messages` conversations with a structured error.
